...
TSPE <- as.data.frame(data.table::fread("/Users/srv/beegfs/scratch/users/l/lykoskou/TE/V3/data_qtls/eqtls/specificity/TUMOR.specificity_All.txt.gz",sep="\t",header=T,stringsAsFactors=F))
SHARED <- as.data.frame(data.table::fread("/Users/srv/beegfs/scratch/users/l/lykoskou/TE/V3/data_qtls/eqtls/specificity/shared_eQTLs.txt.gz",sep=" ", header=T,stringsAsFactors=F))
...
ME <- as.data.frame(data.table::fread("/Users/srv/beegfs/scratch/users/l/lykoskou/TE/V3/data_methylation/data_intersection/syscol_variants_methylations.txt.gz",header=FALSE,stringsAsFactors=F,sep="\t"))
colnames(ME) <- c("var_chr","var_start","var_end","var_id","var_alleles","var_strand","meth_chr","meth_start","meth_end","meth_id","meth_strand")
tmp <- ME[,c(4,10)]
...
t.samples <- as.character(data.table::fread("/Users/srv/beegfs/scratch/users/l/lykoskou/TE/V3/data_expression/stat/syscol_tumor_samples.txt",header=F,stringsAsFactors=F,sep="\t"))[-c(1:6)]


n.samples <- as.character(data.table::fread("/Users/srv/beegfs/scratch/users/l/lykoskou/TE/V3/data_expression/stat/syscol_normal_samples.txt",header=F,stringsAsFactors=F,sep="\t"))[-c(1:6)]
...
data.table::fwrite(tspe.gene.meth, file="/Users/srv/beegfs/scratch/users/l/lykoskou/TE/V3/data_methylation/tspe_gene_meth.txt",sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)
data.table::fwrite(tspe.te.meth, file="/Users/srv/beegfs/scratch/users/l/lykoskou/TE/V3/data_methylation/tspe_te_meth.txt",sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)
data.table::fwrite(shared.gene.meth, file="/Users/srv/beegfs/scratch/users/l/lykoskou/TE/V3/data_methylation/shared_gene_meth.txt",sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)
data.table::fwrite(shared.te.meth, file="/Users/srv/beegfs/scratch/users/l/lykoskou/TE/V3/data_methylation/shared_te_meth.txt",sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)

ALL <- rbind(rbind(rbind(tspe.gene.meth, tspe.te.meth),shared.gene.meth),shared.te.meth)
data.table::fwrite(ALL, file="/Users/srv/beegfs/scratch/users/l/lykoskou/TE/V3/data_methylation/methylation_results_specificityeQTLs.txt",sep="\t",col.names=TRUE,row.names=FALSE,quote=FALSE)
...
gg_te <- ggplot(data=te, aes(x=type, y=abs_med_dif,fill=type))+
    ...
    theme(plot.title=element_text(hjust=0.5,size=18,face="bold"),
        legend.position=0)
gggsave(gg_te,filename="absolute_value_difference_methylation_tumorSpecific_TE_eQTLs.pdf",device="pdf",height=8,width=8)
