...
eqtl <- as.data.frame(data.table::fread("/srv/beegfs/scratch/users/l/lykoskou/TE/V4/data_qtl/eqtl/mapping/tumor/TUMOR.conditional_bestPerRank.txt.gz", header=T,stringsAsFactors=F))
...
pdf("diagnostic_plots_ALL_conditional.pdf")
hist(results$eqtl_int_pval,main="pvalue distribution")
p.qqplot(results$eqtl_int_pval)
dev.off()

write.table(results, file="tumor_specific_eQTL_analysis_results_ALL_conditional.txt",col.names=T,row.names=F,sep="\t",quote=F)
