...
# USEFUL FUNCTIONS # 
TPM <- as.data.frame(data.table::fread("/Users/srv/beegfs/scratch/users/l/lykoskou/TE/V4/data_qtl/teqtl/bn/tumor/TUMOR.BNlearn.ALL.extraINFO.txt.gz", header = TRUE,stringsAsFactors = FALSE,sep="\t"))
...
colnames(TOT) <- c("TinG","TouG","TEupGene","TEdownGene","VinT","VoutT","All")
rownames(TOT) <- c("C","R","I")
TOT$model <- rownames(TOT)

tot <- melt(TOT, id.vars="model")
tot$model <- factor(tot$model, levels=c("C","R","I"))


gg_tumor <- ggplot(data=tot, aes(x=variable, y=value*100,fill=model,label=paste0(round(value*100,0),"%")))+
    geom_bar(stat="identity",color="black",size=1.25)+
    geom_text(position=position_stack(vjust=0.5),size=5)+
    labs(y="Mean probability",x="",title=paste("Tumor (N=",nrow(TPM),")",sep=""))+
...
ggsave(gg_tumor, filename="tumor_te_position_relative_to_gene_ggplot.pdf",device="pdf",height=7,width=12.5)
...
c.f = fisher.test(c) # OR = 0.7006397 Pvalue  1.78e-13
r.f = fisher.test(r) # OR = 1.750945 Pvalue < 2.2e-16
i.f = fisher.test(i) # OR = 0.4579176 Pvalue = 4.971e-15
...
c1.f <- fisher.test(c1) # OR = 1.18765 Pval 0.0002833
r1.f <- fisher.test(r1) # OR = 0.8169689 Pvalue 3.13e-05
i1.f <- fisher.test(i1) # OR = 1.058356 Pvalue  0.4959 
